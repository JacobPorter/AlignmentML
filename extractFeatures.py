...
def quality_features(vector):
    ...
    try:
        skewness = float(sum([math.pow((item - mean) / (variance + 0.0), 3) for item in vector])) / len(vector)
    except ZeroDivisionError:
        skewness = 0
    try:
        kurt = float(sum([math.pow((item - mean) / (variance + 0.0), 4) for item in vector])) / len(vector)
    except ZeroDivisionError:
        kurt = 9/5.0
    diff = []
...
def dna_seq_features(alpha_vector, ignore=False):
    ...
    ent = -1 * sum([dna_count[base] * math.log(dna_count[base], 2) for base in dna_count])
    ...
    if not ignore:
        ...
        return [ent, dust_score, bz2_score, lzma_score, len(alpha_vector)] + freq + run_stats + k_stats
    else:
...
def extractFeatureRow(fastq_record, subportions=3):
    ...
    for i in range(subportions):
        ...
        f2 += dna_seq_features(seq_seq[i * sublength : finallength],
                               ignore = True)
...
def extractFeatures(FASTQ_file):
    ...
    feature_labels = ["seq_id", "entropy", "dust", "bz2", "lzma",
                      "length", "freq_A", "freq_C",
                      "freq_G", "freq_T", "run_mean", "run_variance",
                      "run_max_length", "dkg_2", "rkg_2", "dkg_3", "rkg_3",
                      "dkg_4", "rkg_4", "dkg_5", "rkg_5", "qual_mean",
                      "qual_max", "qual_min", "qual_variance",
                      "qual_skewness", "qual_kurt", "qual_mean_diff"]
    #15 features for each partition
    partition1_labels = ["entropy_1", "dust_1", "bz2_1", "lzma_1",
                         "freq_A_1", "freq_C_1", "freq_G_1",
                         "freq_T_1", "qual_mean_1", "qual_max_1",
                         "qual_min_1", "qual_variance_1", "qual_skewness_1",
                         "qual_kurt_1", "qual_mean_diff_1"]
    partition2_labels = ["entropy_2", "dust_2", "bz2_2", "lzma_2",
                         "freq_A_2", "freq_C_2", "freq_G_2",
                         "freq_T_2", "qual_mean_2", "qual_max_2",
                         "qual_min_2", "qual_variance_2",
                         "qual_skewness_2", "qual_kurt_2", "qual_mean_diff_2"]
    partition3_labels = ["entropy_3", "dust_3", "bz2_3", "lzma_3",
                         "freq_A_3", "freq_C_3", "freq_G_3",
