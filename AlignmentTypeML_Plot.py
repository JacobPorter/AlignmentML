...
def main():
    ...
    sys.stdout.flush()
    doML(getX(args[0]), getY(args[1]), int(options.n_jobs), options.classifier, int(options.datasize), int(options.testsize), options.write_data, options.bayesOpt, options.acqFunc, int(options.numIter))
