...
from sklearn.metrics import classification_report, cohen_kappa_score, confusion_matrix, make_scorer, accuracy_score
...
class EnsembleClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, max_depth=15, max_features=0.5, C_lr = 0.5, activation='relu', alpha=0.999, hidden_layer_sizes=(30, 20, 15, 10), k=10):
    ...
    def fit(self, X, y=None):
        ...
        self.rf = RandomForestClassifier(max_depth=self.max_depth, max_features=self.max_features)
        self.mlp = MLPClassifier(solver='sgd', learning_rate='invscaling', activation=self.activiation, alpha=self.alpha, hidden_layer_sizes=self.hidden_layer_sizes, random_state=1)
...
#         y_gpc = self.gpc.predict(X_scaled)
        sys.stderr.write("Predictions dimensions {0}, {1}, {2}.  y dimensions {3}\n".format(y_rf.shape, y_lr.shape, y_mlp.shape, y.shape))
        sys.stderr.flush()
        X_stacked = np.concatenate((np.array([y_rf]).T, np.array([y_lr]).T, np.array([y_mlp]).T), axis = 1)
        sys.stderr.write("X_stacked dimensions {0}.  y dimensions {1}\n".format(X_stacked.shape, y.shape))
        sys.stderr.flush()
    ...
    def predict(self, X, y=None):
        ...
        y_mlp = self.mlp.predict(X)
#         y_gpc = self.gpc.predict(X_scaled)
        return self.stacker.predict(np.concatenate((np.array([y_rf]).T, np.array([y_lr]).T, np.array([y_mlp]).T), axis = 1))
...
class ColumnSelector(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        if self.delete:
            ...
            return np.delete(X, [i for i in range(X.shape[1]) if i not in self.columns], axis = 1)


def getScaler(X_train):
    return preprocessing.StandardScaler(copy=True, with_mean=True, with_std=True).fit(X_train)
...
def getPipeMLP():
    mlp = MLPClassifier(solver='sgd', learning_rate='invscaling', activation='relu', alpha=1e-5, hidden_layer_sizes=(20, 15), random_state=1)
    estimators = [('scaler', preprocessing.StandardScaler(copy=True, with_mean=True, with_std=True)),
                  ('mlp', mlp)]
    return Pipeline(estimators)
...
features_to_remove = [' length', ' qual_kurt', ' qual_kurt_1', ' qual_kurt_2', ' qual_kurt_3']
...
def checkNumericMatrix(m):
    max_float_1 = 3.402823 * math.pow(10, 37) #max_float = 3.402823 * math.pow(10, 37)
...
def checkNumPyMatrix(mat):
    sys.stderr.write("isnan: " + str(np.any(np.isnan(mat))) + " isfinite: " + str(np.isfinite(mat.all())) + "\n")
...
def doML(X, y_dict, n_jobs, classifier, datasize=None, testsize=100000, write_data = None, bayesOpt = False, acqFunc = "", num_iter = 25,  save_dir="./"):
    global SAVE_DIR
    SAVE_DIR = save_dir
    class_labels = ['Unique', 'Ambig', 'Unmap', 'Filt']
    classifier = classifier.upper()
    sys.stdout.write("Finished with making the features and labels matrices.\n")
    sys.stdout.flush()
    ...
    if datasize != None and datasize <= X.shape[0]:
        X_train = X[: datasize]
        y_train = y[: datasize]
    sys.stdout.write("The size of X_train is %s, and the size of y_train is %s.\n" % (str(X_train.shape), str(y_train.shape)) )
    if testsize != None:
        X_test = X[datasize : datasize + testsize]
        y_test = y[datasize : datasize + testsize]
    sys.stdout.write("The size of X_test is %s, and the size of y_test is %s.\n" % (str(X_test.shape), str(y_test.shape)) )
    if write_data != None:
        scaler = preprocessing.StandardScaler(copy=True, with_mean=True, with_std=True)
        scaler.fit(X_train)
        np.save(write_data + '_' + str(datasize) + '_' + 'X_train_scaled.npy', scaler.transform(X_train))
        np.save(write_data + '_' + str(testsize) + '_' + 'X_test_scaled.npy', scaler.transform(X_test))
        np.save(write_data + '_' + str(datasize) + '_' + 'y_train.npy', y_train)
    ...
    f_test_features = sorted(zip(feature_labels, f_test, pval), key=lambda x: x[1], reverse=True)
    print("")
    print("")
    ...
    if 'RAND' in classifier:
        result_rand = doRand(X_train, y_train, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt, acqFunc)
    ...
    if 'MLP' in classifier:
        ...
        sys.stdout.flush()
    if 'LR' in classifier:
        result_lr = doLogisticRegression(X_train, y_train, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt, acqFunc, num_iter)
        sys.stdout.write("Finished with logistic regression.\n")
        sys.stdout.flush()
    if 'ENSEMBLE' in classifier:
        result_ensemble = doEnsemble(X_train, y_train, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt, acqFunc, num_iter)
        sys.stdout.write("Finished with ensemble.\n")
        sys.stdout.flush()
    if 'SVC' in classifier:
        result_svc = doSVC(X_train, y_train, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt, acqFunc, num_iter)
...
def executeML(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, pipe, parameters, ml_type, bayesOpt = False, search_space = None, n_iter = 32, acq_func=""):
    if bayesOpt:
        ...
        cv = BayesSearchCV(pipe, search_space, verbose = verbose, n_iter = n_iter, n_jobs = n_jobs, optimizer_kwargs = optimizer_kwargs, scoring = kappa_scorer)
    else:
        cv = GridSearchCV(pipe, parameters, verbose = verbose, n_jobs = n_jobs, scoring = kappa_scorer)
    tick1 = datetime.datetime.now()
    cv.fit(X, y)
    tick2 = datetime.datetime.now()
    ...
    tick4 = datetime.datetime.now()
    print("{0} test set accuracy: {1}".format(ml_type, accuracy_score(y_test, y_predict)))
    print("{0} test set cohen kappa: {1}".format(ml_type, cohen_kappa_score(y_test, y_predict)))
    print("{0} prediction time: {1}".format(ml_type, tick4 - tick3))
    print(classification_report(y_test, y_predict, target_names=class_labels, digits=8))
    cnf_matrix = confusion_matrix(y_test, y_predict)
    print("")
    print(cnf_matrix)
    pickle.dump(cnf_matrix, open(os.path.join(SAVE_DIR, "cnf_matrix.best." + ml_type + ".pck"), 'wb'))
    pickle.dump(cv.best_estimator_, open(os.path.join(SAVE_DIR, "classifier.best." + ml_type + ".pck"), 'wb'))
    return cv.best_estimator_

def doRand(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt = False, acqFunc = '', num_iter = 25):
    #Best: 'lr__C': 0.74
    ...
    print(cnf_matrix)
    pickle.dump(cnf_matrix, open(os.path.join(SAVE_DIR, "cnf_matrix.best." + ml_type + ".pck"), 'wb'))
    pickle.dump(random_classifier, open(os.path.join(SAVE_DIR, "classifier.best." + ml_type + ".pck"), 'wb'))
    return random_classifier

def doRandomForest(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt = False, acqFunc = '', num_iter = 25):
...
#                   "rf__max_features": ['sqrt', None, 0.5]}
    ...
    parameters = {"rf__max_depth": [i for i in range(5, 60, 5)],
                  "rf__max_features": [i/10.0 for i in range(1, 11)]}
    best_estimator = executeML(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, getPipeRF(), parameters, "RF" + acqFunc, bayesOpt = bayesOpt, search_space = search_space, n_iter = num_iter, acq_func = acqFunc)
    try:
...
def doLogisticRegression(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt = False, acqFunc = '', num_iter = 25):
    ...
    parameters = {"lr__C": [i/50.0 for i in range(1, 51)]} #[0.01, 0.25, 0.5, 0.75, 1.0]
    best_estimator = executeML(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, getPipeLR(), parameters, "LR", bayesOpt = bayesOpt, search_space = search_space, n_iter = num_iter, acq_func = acqFunc)
    try:
...
def doSVC(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt = False, acqFunc = '', num_iter = 5):
    """
    ...
    parameters = {'svc__C':[0.01, 0.25, 0.5, 0.75, 1.0], #[i/50.0 for i in range(1, 51)]
                  }
...
#                   'svc__C':[0.5],
#                   } #'C':[1, 0, 0.1, ]
    best_estimator = executeML(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, getPipeSVC(), parameters, "SVC", bayesOpt = bayesOpt, search_space = search_space, n_iter = num_iter, acq_func = acqFunc)
    try:
        imp = best_estimator.named_steps['svc'].coef_
        print(str(imp))
    except AttributeError:
        print("How to get the feature importances for SVC?")
    print("SVC number of support vectors: {0}".format(best_estimator.named_steps['svc'].n_support_))
    return best_estimator
...
def doEnsemble(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt = False, acqFunc = '', num_iter = 25):
    search_space = {}
    parameters = {'ens__max_depth':[15],
    ...
    return executeML(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, getPipeEnsemble(), parameters, "Ensemble", bayesOpt = bayesOpt, search_space = search_space, n_iter = num_iter, acq_func = acqFunc)

def doMLP(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, bayesOpt = False, acqFunc = '', num_iter = 25):
    #Best parameters: alpha=0.999, hidden_layer_sizes=(30, 20, 15, 10)
    search_space = {"mlp__alpha": Real(.001, 1.0, prior='log-uniform'), "mlp__hidden_layer_sizes":Categorical([(30, 20, 15, 10)])}
    parameters = {'mlp__alpha':[i/10.0 for i in range(1, 11)],#[1e-5, 0.25, 0.5, 0.999],
                  'mlp__hidden_layer_sizes':[(30, 20, 15, 10)],#[(30, 20, 15, 10), (30, 20, 15, 10, 5), (40, 30, 20, 15, 10, 5), (40, 40, 40, 40, 40, 40)],#[(20, 15), (20, 20), (20, 15, 5), (30, 20), (30, 20, 15, 10)],
                  }
    best_estimator = executeML(X, y, X_test, y_test, n_jobs, feature_labels, class_labels, getPipeMLP(), parameters, "MLP", bayesOpt = bayesOpt, search_space = search_space, n_iter = num_iter, acq_func = acqFunc)
    try:
...
def getX(filelocation):
    ...
    feature_labels = X_fd.readline().strip().replace("]", "").replace("[", "").replace("'", "").split(",")
...
def main():
    ...
    p.add_option('--numIter', '-i', help='The number of iterations (samples) for Bayesian optimization.  [default: %default]', default = 25)
    p.add_option('--save_dir', '-d', help='The directory to save models too.', default='./')
    ...
    sys.stdout.write("Executing AlignmentTypeML on feature file {0} and label file {1}.  There will be {2} processes.  The training data size is {3}, and the test size is {4}.\n".format(args[0], args[1], options.n_jobs, options.datasize, options.testsize ))
    sys.stdout.write("Classifier: {0}\nBayesian Optimization: {1}, {2}, {3}.\n".format(options.classifier, options.bayesOpt, options.acqFunc, options.numIter))
    sys.stdout.flush()
    doML(getX(args[0]), getY(args[1]), int(options.n_jobs), options.classifier, int(options.datasize), int(options.testsize),
         options.write_data, options.bayesOpt, options.acqFunc, int(options.numIter),
         options.save_dir)
    later = datetime.datetime.now()
    sys.stdout.write("The AlignmentTypeML was started at %s and took %s time.\n" % (str(now), str(later - now)))
